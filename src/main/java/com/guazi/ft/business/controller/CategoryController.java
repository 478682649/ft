...
import com.guazi.ft.service.CategoryService;
...
public class CategoryController {
	...
	@ApiImplicitParams({
			@ApiImplicitParam(name = "id", value = "分类id", required = true, dataType = Swagger2.DATA_TYPE_INT, paramType = Swagger2.PARAM_TYPE_QUERY, example = "0"),
			@ApiImplicitParam(name = "name", value = "分类名称", required = true, dataType = Swagger2.DATA_TYPE_STRING, defaultValue = "默认分类名称", paramType = Swagger2.PARAM_TYPE_QUERY)
	})
	@ApiResponses({
			@ApiResponse(code = RestResult.SUCCESS_CODE, message = Swagger2.MESSAGE_SUCCESS, response = RestResult.class),
			@ApiResponse(code = RestResult.ERROR_CODE, message = Swagger2.MESSAGE_ERROR)
	})
	/**
	 * 根据id修改分类信息
	 *
